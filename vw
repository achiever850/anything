...
SELECT
    ...
    fact.hashvalue,
    fact.gpracctgsid,
    -- New columns from rule_periods (enhancements)
    TO_DATE(rp.PD_START_DT, 'YYYY-MM-DD') AS "Paid Start Date",
    TO_DATE(rp.PD_END_DT, 'YYYY-MM-DD') AS "Paid End Date",
    TO_NUMBER(TO_CHAR(TO_DATE(rp.PD_START_DT, 'YYYY-MM-DD'), 'YYYYMMDD')) AS "PayCalendarDateKey"
FROM
...
-- New Join with rule_periods
JOIN
    hrpas_pay_history_staging.rule_periods rp
